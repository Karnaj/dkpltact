...
let get_new_name (set: string * string) (set_index: ((string * string) * int) list) = 
  match List.assoc_opt set set_index with 
  | None -> (Printf.sprintf "%c%d" (String.get (snd set) 0) 0, (set, 1)::set_index)
  | Some(i) -> (Printf.sprintf "%c%d" (String.get (snd set) 0) i, (set, i + 1)::(List.remove_assoc set set_index))
...
let rec parse_element name_assoc x = match x with
| T.Const (_, cst) -> 
...
let rec parse_proposition set_index name_assoc p = match p with
  ...
  | T.App (T.Const (_, cst), t1, [t2]) when is_imp cst ->
    Ast.Implication(parse_proposition set_index name_assoc t1, parse_proposition set_index name_assoc t2)
  | T.App (T.Const (_, cst), t1, [t2]) when is_and cst ->
    Ast.Conjonction(parse_proposition set_index name_assoc t1, parse_proposition set_index name_assoc t2)
  | T.App (T.Const (_, cst), t1, [t2]) when is_or cst ->
    Ast.Disjonction(parse_proposition set_index name_assoc t1, parse_proposition set_index name_assoc t2)
  ...
  | T.App (T.Const (_, cst), T.Const(_, set), [t1; t2]) when is_eq cst ->
    Ast.Equality(pair_string_of_name set, parse_element name_assoc t1, parse_element name_assoc t2)
  | T.App (T.Const (_, cst), T.Const(_, set), [t1; t2]) when is_neq cst ->
      Ast.NotEquality(pair_string_of_name set, parse_element name_assoc t1, parse_element name_assoc t2)
...
let rec parse_predicate_definition set_index name_assoc te = match te with 
...
let rec parse_function_definition set_index name_assoc te = match te with 
  | T.Lam(_, id, Some(T.App(T.Const(_, cst), T.Const(_, set), _)), t) when is_el cst -> 
    let set = pair_string_of_name set in
    let id = B.string_of_ident id in
    let elname, set_index = get_new_name set set_index in 
...
let rec parse_proof set_index name_assoc p ctx locals = match p with
  ...
  | T.Lam(_, id, Some(T.App(T.Const(_, cst), T.Const(_, set), [])), t) when is_el cst ->
      ...
      let name_assoc = (id, elname)::(List.remove_assoc id name_assoc) in
      let locals = (elname, Ast.ElementC(set_name))::(List.remove_assoc elname locals) in
  ...
  | T.App(T.Const(_, cst), a, b::aprf::bprf::rest) when is_and_intro cst ->
    ...
    parse_proof_with_other_args set_index name_assoc (Ast.AndIntro(a, b, aprf, bprf)) (Ast.Conjonction(a, b)) ctx locals rest

  | T.App(T.Const(_, cst), b, a::bprf::rest) when is_or_intro_l cst ->
      let a = parse_proposition set_index name_assoc a in
      let b = parse_proposition set_index name_assoc b in
      let (prf, _) = parse_proof set_index name_assoc bprf ctx locals in 
      parse_proof_with_other_args set_index name_assoc (Ast.OrIntroL(a, b, prf)) (Ast.Disjonction(a, b)) ctx locals rest

  | T.App(T.Const(_, cst), b, a::aprf::rest) when is_or_intro_r cst ->
      let a = parse_proposition set_index name_assoc a in
      let b = parse_proposition set_index name_assoc b in
      let (prf, _) = parse_proof set_index name_assoc aprf ctx locals in 
      parse_proof_with_other_args set_index name_assoc (Ast.OrIntroR(a, b, prf)) (Ast.Disjonction(a, b)) ctx locals rest
  ...
  | T.App(T.Const(_, cst), T.Const(_, set), T.Lam(_, x, _,predicate)::wit::proof_ex::rest) when is_ex_intro cst ->
    ...
    let pred = (set_name, elname, p) in 
    parse_proof_with_other_args set_index name_assoc (Ast.ExIntro(pred, wit, prf)) (Ast.Exists(pred)) ctx locals rest
  ...
  | T.App(T.Const(_, cst), p1, proof_false::rest) when is_false_elim cst ->
      ...
      parse_proof_with_other_args set_index name_assoc (Ast.FalseElim(prf)) p ctx locals rest
  

  
  | T.App(T.Const(_, cst), p1, p2::p::proof_p::proof_and::rest) when is_and_ind cst ->
    ...
    let (proof_and, _) = parse_proof set_index name_assoc proof_and ctx locals in
    let h1_name, set_index = get_new_name ("", "H") set_index in
    let h2_name, set_index = get_new_name ("", "H") set_index in
    let proof_p = begin match proof_p with 
      ...
      | _ -> 
        let (proof_p, _) = parse_proof set_index name_assoc proof_p ctx locals in
        let newprop, _ = get_new_name ("", "H") set_index in
        Ast.Cut(
            Ast.Implication(p1, Ast.Implication(p2, p)), 
            proof_p, 
            newprop, 
            Ast.Apply(newprop, [Ast.TProof(Ast.Assumption(h1_name)); Ast.TProof(Ast.Assumption(h2_name))]))
      end in
    parse_proof_with_other_args set_index name_assoc (Ast.AndInd(h1_name, p1, h2_name, p2, proof_and, p, proof_p)) p ctx locals rest

    | T.App(T.Const(_, cst), p, q::r::proof_r::proof_and::rest) when is_and_ind_r cst ->
      let p = parse_proposition set_index name_assoc p in 
      ...
      let (proof_and, _) = parse_proof set_index name_assoc proof_and ctx locals in 
      let proof_r = begin match proof_r with 
        | T.Lam(_, h, _, proof_r) ->
        ...
        |  _ ->
          let (proof_r, _) = parse_proof set_index name_assoc proof_r ctx locals in
          let newprop, _ = get_new_name ("", "H") set_index in
          Ast.Cut(
            Ast.Implication(q, r), 
            proof_r, 
  ...
  | T.App(T.Const(_, cst), p, q::r::proof_r::proof_and::rest) when is_and_ind_l cst ->
    let p = parse_proposition set_index name_assoc p in 
    let q = parse_proposition set_index name_assoc q in
    let r = parse_proposition set_index name_assoc r in
    let hname , set_index = get_new_name ("", "H") set_index in
    ...
    | T.App(T.Const(_, cst), p1, p2::p::proof1::proof2::proof_or::rest) when is_or_elim cst ->
      let h1_name, set_index = get_new_name ("", "H") set_index in
      let h2_name, set_index = get_new_name ("", "H") set_index in
      let p1 = parse_proposition set_index name_assoc p1 in 
      let p2 = parse_proposition set_index name_assoc p2 in
      let p = parse_proposition set_index name_assoc p in
      let proof1 = begin match proof1 with 
        | T.Lam(_, h1, _, proof1) -> 
            let h1 = B.string_of_ident h1 in
            let name_assoc = (h1, h1_name)::(List.remove_assoc h1 name_assoc) in
            fst (parse_proof set_index name_assoc proof1 ctx locals)
        | _ -> 
          let (proof1, _) = parse_proof set_index name_assoc proof1 ctx locals in
          let newprop, _ = get_new_name ("", "H") set_index in
          Ast.Cut(
      ...
      let proof2 = begin match proof2 with 
        | T.Lam(_, h2, _, proof2) -> 
            let h2 = B.string_of_ident h2 in
            let name_assoc = (h2, h2_name)::(List.remove_assoc h2 name_assoc) in
            fst (parse_proof set_index name_assoc proof2 ctx locals)
        | _ -> 
          let (proof2, _) = parse_proof set_index name_assoc proof2 ctx locals in
          let newprop, _ = get_new_name ("", "H") set_index in
          Ast.Cut(
      ...
      parse_proof_with_other_args set_index name_assoc
        (Ast.OrInd(p1, p2, p, h1_name, proof1, h2_name, proof2, proof_or)) p ctx locals rest 
  

  (* forall wit, setn, h(predicate wit) => proofp *)
  | T.App(T.Const(_, cst), T.Const(_, setn), T.Lam(_, x, _,predicate)::p::proof_p::proof_ex::rest) when is_ex_elim cst ->
    ...
    let proof_p = begin match proof_p with
      | T.Lam(_, wit, _, T.Lam(_, h, _, proof_p)) -> 
         ...
         let name_assoc = (wit, wit_name)::(List.remove_assoc wit name_assoc) in 
         let h = B.string_of_ident h in  
      ...
      | _ -> 
        let (proof_p, _) = parse_proof set_index name_assoc proof_p ctx locals in
        let newprop, _ = get_new_name ("", "H") set_index in
        let pred = (set, wit_name, Ast.Implication(instantiate xname pred (Ast.ElementCst(wit_name)), p)) in
        Ast.Cut(
            Ast.Forall(pred), 
            proof_p, 
            newprop, 
            Ast.Apply(newprop, [Ast.TElement(Ast.ElementCst(wit_name)); Ast.TProof(Ast.Assumption(h_name))]))
          end in 
    parse_proof_with_other_args set_index name_assoc
    (Ast.ExInd((set, xname, pred), p, proof_ex, wit_name, h_name, proof_p)) p ctx locals rest
  ...
  | T.App(T.Const(_, cst), T.Const(_, set_name), x::y::z::proof_eq1::proof_eq2::rest) when is_eq_trans cst ->
    ...
    let (proof_eq1, _) = parse_proof set_index name_assoc proof_eq1 ctx locals in
    let (proof_eq2, _) = parse_proof set_index name_assoc proof_eq2 ctx locals in 
    parse_proof_with_other_args set_index name_assoc
...
and parse_proof_with_other_args set_index name_assoc prf prop ctx locals args = match (prop, args) with
  ...
  | (Ast.False, prop::rest) ->
    ...
    let _, prop = parse_proof_with_other_args set_index name_assoc 
      Ast. T  
      prop ctx locals rest in 
  ...
  | (Ast.Conjonction(p1, p2), p::prfp::rest) ->
    ...
    let h2_name, set_index = get_new_name ("", "H") set_index in
    let p = parse_proposition set_index name_assoc p in
    let proof_p = begin match prfp with 
      | T.Lam(_, h1, _, T.Lam(_, h2, _, proof_p)) -> 
        let h1 = B.string_of_ident h1 in
        let h2 = B.string_of_ident h2 in 
        let name_assoc = (h1, h1_name)::(List.remove_assoc h1 name_assoc) in
    ...
    parse_proof_with_other_args set_index name_assoc (Ast.AndInd(h1_name, p1, h2_name, p2, prf, p, proof_p)) p ctx locals rest
  ...
  | (Ast.Equality(set, x, y), T.Lam(_, z, _,predicate)::prf_x::rest) -> 
    ...
    parse_proof_with_other_args set_index name_assoc prf_y (instantiate id p y) ctx locals rest
...
and instantiate (id: string) (p: Ast.proposition) (t: Ast.element) = match p with
