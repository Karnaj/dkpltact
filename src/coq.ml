...
let rec string_of_coq_prop prop = match prop with 
  ...
  | Forall(l, prop) -> 
    let args = List.fold_left (fun s var -> Printf.sprintf "%s (%s: %s)" s (snd var) (string_of_name (fst var))) "" l in
...
let rec string_of_term t = match t with 
  ...
  | Ast.TElement(t) ->
      ...
      if is_atomic_element(t) then
        string_of_coq_element t
      else
        Printf.sprintf "(%s)" (string_of_coq_element t)
  | _ -> failwith "Nope"
...
| Ast.Assumption(x) -> 
    [Command (Printf.sprintf "exact %s." x)]
...
| Ast.AndInd(hp, _, hq, _, GlobalAssumption(h), _, prf) ->
    let str = Printf.sprintf "destruct %s as [%s %s]." (string_of_name h) hp hq in
    let l = coq_string_step_of_proof prf ctx in
    (Command str)::l
...
| Ast.AndInd(hp, p, hq, q, prfand, _, prf) ->
    let str1 = Printf.sprintf "assert (%s) as %s." (string_of_coq_prop (Conjonction(translate_proposition p, translate_proposition q))) hp in
...
| Ast.AndIndRight(p, hq, q, prfand, _, prf) ->
    let str1 = Printf.sprintf "assert (%s) as %s." (string_of_coq_prop (Conjonction(translate_proposition p, translate_proposition q))) hq in
...
| Ast.AndElimRight(p, q, prf) ->
  let str1 = Printf.sprintf "assert (%s) as Hand." (string_of_coq_prop (Conjonction(translate_proposition p, translate_proposition q))) in
...
| Ast.OrInd(_, _, _, hleft, proofleft, hright, proofright, Ast.GlobalAssumption(x)) ->
  let str1 = Printf.sprintf "destruct %s as [%s|%s]." (string_of_name x) hleft hright in
  let strleft = coq_string_step_of_proof proofleft ctx in 
  let strright = coq_string_step_of_proof proofright ctx in
  [Command(str1) ; Step(strleft) ; Step(strright)]
| Ast.OrInd(p, q, _, hleft, proofleft, hright, proofright, prf) ->
  let str = Printf.sprintf "assert (%s) as %s." (string_of_coq_prop (Disjonction(translate_proposition p, translate_proposition q))) hleft in
  let str1 = Printf.sprintf "destruct %s as [%s|%s]." hleft hleft hright in
  let strleft = coq_string_step_of_proof proofleft ctx in 
  let strright = coq_string_step_of_proof proofright ctx in
  let strprf = coq_string_step_of_proof prf ctx in
  [Command(str) ; Step(strprf) ; Step([Command(str1) ; Step(strleft) ; Step(strright)]) ]
| Ast.ExIntro(_, x, prf) ->
  let str = Printf.sprintf "exists %s." (string_of_coq_element (translate_element x)) in
...
| Ast.ExInd(_, _,  Ast.GlobalAssumption(x), wit_name, h, proof_p) ->
  let str =  Printf.sprintf "destruct %s as [%s %s].\n" (string_of_name x) wit_name h in 
  let str1 = coq_string_step_of_proof proof_p ctx in 
  Command(str)::str1
| Ast.ExInd(pred, _,  prfexists, wit_name, h, proof_p) ->
    let stror = Printf.sprintf "assert (%s) as %s." (string_of_coq_prop (translate_proposition (Ast.Exists pred))) h in
    let str =  Printf.sprintf "destruct %s as [%s %s].\n" h wit_name h in 
    let str1 = coq_string_step_of_proof proof_p ctx in 
...
| Ast.ForallIntro((_, x, _), proof) ->
    ...
    let str = Printf.sprintf "intros%s." (List.fold_left (fun str arg -> Printf.sprintf "%s %s" str arg) "" args) in
    Command(str)::prf
| Ast.ForallElim(_, Ast.Assumption(h), x) ->
  let str = Printf.sprintf "apply %s with (%s)." h (string_of_coq_element (translate_element x)) in
  [Command(str)]
...
| Ast.Cut(p, prfp, h, prf) ->
  let strcut = Printf.sprintf "assert %s as %s." (coq_string_of_prop p) h in 
  let strp = coq_string_step_of_proof prfp ctx in
  let str = coq_string_step_of_proof prf ctx in
...
| ((_, x), Ast.Element(set)) -> Printf.sprintf "Parameter %s: %s." x (string_of_name set)
| ((_, f), Ast.FunctionSymbol(args, ret)) -> Printf.sprintf "Parameter %s: %s -> %s." f (string_of_paramaters_type args) (string_of_name ret) 
| ((_, f), Ast.PredicateSymbol(args)) -> Printf.sprintf "Parameter %s: %s -> Prop." f (string_of_paramaters_type args)
